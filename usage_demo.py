...
# 2. Örnek bir tensör oluştur (modelin latent katmanı gibi düşün)
latent_state = torch.randn(10, requires_grad=True)

# 3. Sentinel üzerinden geçir
safe_output = sentinel(latent_state)

print("Original latent state:", latent_state.detach().numpy())
print("Safe (stabilized) output:", safe_output.detach().numpy())
